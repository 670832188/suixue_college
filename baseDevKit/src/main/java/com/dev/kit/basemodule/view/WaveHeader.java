...
public class WaveHeader extends View implements IRefreshView {
    ...
    public WaveHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            final TypedArray arr = context.obtainStyledAttributes(attrs, me.dkzwm.widget.srl.R.styleable.IRefreshView, 0, 0);
            @RefreshViewStyle
            int style = arr.getInt(me.dkzwm.widget.srl.R.styleable.IRefreshView_sr_style, mStyle);
            mStyle = style;
            arr.recycle();
        }
